...
- `artifacts/mobile/lib/promoCodes.ts` — promo/comp code catalog (4 codes). Each is `percent_off` (display discount) or `grant_tier` (free entitlement). `lookupPromo(raw)` matches case-insensitively. **Codes:** `MAISON20` (20% off), `MAISON30` (30% off), `MAISON50` (50% off), `DIAMONDHOUSE` (free Diamond tier)
- `artifacts/mobile/lib/promoSettings.ts` — AsyncStorage-backed redeemed-promo store (mirrors `affiliateSettings.ts`: cached/loaded/listeners/subscribe + `useSyncExternalStore`-stable `getPromoState()`). For built-in codes persists only the code string (effect re-derived from `promoCodes.ts`); for server (admin-created) codes also persists an effect `snapshot` taken at redeem time. `redeemPromoCode` tries built-ins first, then falls back to `lookupServerPromo()`
- `artifacts/mobile/lib/promoAdmin.ts` — promo-admin API client + AsyncStorage admin-key store (`loadAdminKey`/`saveAdminKey`/`clearAdminKey`, device-only). Public `lookupServerPromo()` (no key); admin `list/create/update/deletePromoCode` send `x-admin-key`. `PromoAdminError` carries `.status`
- `artifacts/mobile/app/admin-promos.tsx` — owner-only admin screen: admin-key gate → create/edit/activate/delete custom server codes; built-in 4 shown read-only. Discreet entry from `app/affiliate-settings.tsx` (owner tools)
- `artifacts/api-server/src/routes/promo.ts` — public GET `/promo/lookup` (active codes only); admin GET/POST `/promo/codes` + PATCH/DELETE `/promo/codes/{id}` gated by `requireAdmin` (`x-admin-key` === `ADMIN_PROMO_KEY` env; 503 if unset, 401 if mismatch). 409 on duplicate code, 404 on missing
- `lib/db/src/schema/promoCodes.ts` — `promo_codes` table (code unique, kind, percent?, tier?, label, active, timestamps). Admin-created codes only — built-ins stay in `promoCodes.ts`. Required env for admin: `ADMIN_PROMO_KEY` secret
...
- **Promo codes layer ON TOP of RC-derived entitlements.** `grant_tier` codes (e.g. `DIAMONDHOUSE` → free Diamond) override the tier UP only — `tier = max(rcTier, promoGrantedTier)` by rank in `EntitlementsContext`; a promo can never downgrade a real RC purchase. Redeeming a grant code also POSTs `/subscriptions/sync` so the server daily-cap mirror honours it; clearing it re-syncs back to the RC tier. `percent_off` codes (`MAISON20/30/50`) are **display-only** — they restyle the paywall prices (strikethrough → discounted) but do NOT change what the App Store / Play actually charges (that needs store-configured offer codes). Pre-launch this is acceptable; the comp-tier grant is a real in-app entitlement.
...
- **Every displayed outfit must be COMPLETE — no partial outfits.** `isCompleteOutfit(pieces, genderKey)` in `outfitEngine.ts` is the single source of truth and gates every look-builder. Men/unisex: top + bottom + shoes (outerwear optional, bag recommended). Women: (dress OR top+bottom) + shoes + **bag REQUIRED** (handbag). `generateLooks` (both dress + separates branches, plus the brand-lock and ultra fallbacks) and `generateLookFromAIPlan` all run this gate as their final check. The AI resolver additionally BACKFILLS missing required categories from `CATALOG` (HARD gender+season respected) before gating, so an under-specified AI plan still yields a complete look or none. This composes WITH the budget hard-cap: a required bag that can't fit budget drops the look (honest empty-state) rather than showing a partial or over-budget outfit. Requiring a women's bag means a thinly-stocked brand-lock with no bag legitimately yields zero looks — that's the intended honest empty-state, not a bug.
- **Promo codes are TWO independent sources, merged at redeem time.** (1) The 4 built-in codes live in `promoCodes.ts` (compiled into the bundle, work OFFLINE, can't be edited at runtime). (2) Admin-created codes live in the `promo_codes` DB table, managed via `app/admin-promos.tsx` and `/api/promo/codes`, and work for EVERY customer. `redeemPromoCode` checks built-ins first (instant, offline) then falls back to `lookupServerPromo()`. Server codes persist an effect SNAPSHOT at redeem time so a later owner edit/deactivate doesn't retroactively revoke a customer's already-redeemed grant (normal promo-code semantics: edits affect future redemptions only). The admin CRUD is gated by `ADMIN_PROMO_KEY` (server secret); the owner types the same value into the in-app key gate once (stored device-only in AsyncStorage). Public `/promo/lookup` returns ACTIVE codes only — deactivating is a soft kill.
...
- **Never use expo-router `<Link asChild>` wrapping a `Pressable`/`View` that carries a `style={[...]}` ARRAY on web.** On web `Link` renders a real `<a>` DOM host element and `asChild` merges the child's RN style array onto it; the `<a>` host does NOT flatten arrays, so React DOM throws the "Failed to set an indexed property [0] on CSSStyleDeclaration" crash (this was the long-standing desktop-web crash — confirmed in `DesktopNav.tsx`). Navigate imperatively instead: `const router = useRouter()` + plain `<Pressable onPress={() => router.push(href)}>`. Array styles on `Pressable`/`Text`/`View` are fine — they flatten natively; the danger is only when an array reaches a DOM host element. Reproduces in the dev preview at a ≥1024px viewport on a tab route (`/explore` etc.) — `/` redirects to onboarding first.
