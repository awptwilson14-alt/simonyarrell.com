...
- `artifacts/mobile/lib/promoSettings.ts` — AsyncStorage-backed redeemed-promo store (mirrors `affiliateSettings.ts`: cached/loaded/listeners/subscribe + `useSyncExternalStore`-stable `getPromoState()`). Only persists the code string; effect is always re-derived from `promoCodes.ts`
