...
- **Any `style={[...]}` array passed DIRECTLY to a third-party web shim (`Image` from expo-image, `LinearGradient` from expo-linear-gradient, `BlurView`, etc.) must be wrapped in `StyleSheet.flatten([...])`.** This is the same root cause as the wrapper rule above — RN-Web's `preprocess()` iterates style objects with `for...in`; an unflattened array's numeric keys ("0","1") become invalid CSS property setters. Pure-RN sinks (View, Pressable, Text) are safe — RN flattens before paint. Audited surfaces using this pattern: `TrendCard`, `LookCard`, `GoldButton`, `tryon.tsx`, `activity.tsx`, `ResilientImage`. Adding new ones means following the same flatten convention from day one.
- **HeroAudio web autoplay is unlocked by a one-shot document gesture listener.** Browsers (desktop + mobile Safari/Chrome) reject audible autoplay until the user produces a gesture, so the player mounts muted on web. A one-time `pointerdown/touchstart/click/keydown` listener on `document` flips `player.muted=false` + `player.play()` on the user's first interaction anywhere on the page (only when `defaultMuted=false`). The visible speaker icon remains the manual override afterwards. Without this, the only way to get sound on web was to find and tap the tiny corner speaker icon — which almost no user discovered.
