...
- **Wrappers that forward `style` into third-party web components (expo-image, LinearGradient, etc.) must `StyleSheet.flatten(style)` before re-composing.** RN-Web's style pipeline iterates style objects with `for...in`; if an unflattened array reaches `preprocess()`, its numeric keys ("0","1") get treated as CSS property names and trigger the indexed-property setter error. See `ResilientImage` for the canonical defensive pattern. Pure-RN wrappers (View → View) don't need this — React Native's own renderer flattens before paint.
