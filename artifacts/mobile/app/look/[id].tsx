...
// back to a curated luxury tile — brand monogram + category icon + color dot
// — so the cell never renders as an empty/broken square. The fallback is
// deliberately editorial (serif monogram, gold accents) so it reads as
// "intentional, no-photo presentation" rather than a missing-image error.
function categoryIcon(cat?: string): React.ComponentProps<typeof Feather>["name"] {
...
function brandMonogram(brand?: string): string {
  if (!brand) return "·";
  // Pick first letter of first two significant words (skip "the", "x", "&").
  const words = brand
    .replace(/[^\w\s&]/g, " ")
    .split(/\s+/)
...
// Map common garment color names to a representative hex for the corner swatch.
// Keys are normalized to lowercase; multi-word colors like "off white" handled.
...
function colorNameToHex(name?: string, fallback = "#6B7280"): string {
  if (!name) return fallback;
  const key = name.trim().toLowerCase().replace(/[-_/]/g, " ");
  if (COLOR_NAME_TO_HEX[key]) return COLOR_NAME_TO_HEX[key];
  // Try last word (e.g., "off white" → "white", "dark grey" → "grey").
